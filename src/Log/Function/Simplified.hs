...
reportMessageS :: MonadIO m => LogPrefix -> String  -> m ()
reportMessageS p s = orderedMessage (LogMessage p LOG_ZONE s)

-- Simple Messages, Equivalent to Print but in ManiLog Style
-- A chained message brick, pass anything to it, and manage it's order outside of Mani-Log
debugMessageS :: MonadIO m => LogPrefix -> String  -> m ()
debugMessageS p s = orderedMessage (LogMessage p LOG_DEBUG s)

-- An unchained message; Be careful this may break formatting if put in a list with an orderedMessage.
eventMessageS :: MonadIO m => String -> m ()
eventMessageS s = eventMessage (LogMessage LOG_HEAD LOG_EVENT s)
